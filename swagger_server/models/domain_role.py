# coding: utf-8

from __future__ import absolute_import
from .base_model_ import Model
from datetime import date, datetime
from typing import List, Dict
from ..util import deserialize_model


class DomainRole(Model):
    """
    NOTE: This class is auto generated by the swagger code generator program.
    Do not edit the class manually.
    """
    def __init__(self, domain_id: int=None, role_id: int=None, grant_implicitly: bool=None, created_at: datetime=None, updated_at: datetime=None):
        """
        DomainRole - a model defined in Swagger

        :param domain_id: The domain_id of this DomainRole.
        :type domain_id: int
        :param role_id: The role_id of this DomainRole.
        :type role_id: int
        :param grant_implicitly: The grant_implicitly of this DomainRole.
        :type grant_implicitly: bool
        :param created_at: The created_at of this DomainRole.
        :type created_at: datetime
        :param updated_at: The updated_at of this DomainRole.
        :type updated_at: datetime
        """
        self.swagger_types = {
            'domain_id': int,
            'role_id': int,
            'grant_implicitly': bool,
            'created_at': datetime,
            'updated_at': datetime
        }

        self.attribute_map = {
            'domain_id': 'domain_id',
            'role_id': 'role_id',
            'grant_implicitly': 'grant_implicitly',
            'created_at': 'created_at',
            'updated_at': 'updated_at'
        }

        self._domain_id = domain_id
        self._role_id = role_id
        self._grant_implicitly = grant_implicitly
        self._created_at = created_at
        self._updated_at = updated_at

    @classmethod
    def from_dict(cls, dikt) -> 'DomainRole':
        """
        Returns the dict as a model

        :param dikt: A dict.
        :type: dict
        :return: The domain_role of this DomainRole.
        :rtype: DomainRole
        """
        return deserialize_model(dikt, cls)

    @property
    def domain_id(self) -> int:
        """
        Gets the domain_id of this DomainRole.

        :return: The domain_id of this DomainRole.
        :rtype: int
        """
        return self._domain_id

    @domain_id.setter
    def domain_id(self, domain_id: int):
        """
        Sets the domain_id of this DomainRole.

        :param domain_id: The domain_id of this DomainRole.
        :type domain_id: int
        """
        if domain_id is None:
            raise ValueError("Invalid value for `domain_id`, must not be `None`")

        self._domain_id = domain_id

    @property
    def role_id(self) -> int:
        """
        Gets the role_id of this DomainRole.

        :return: The role_id of this DomainRole.
        :rtype: int
        """
        return self._role_id

    @role_id.setter
    def role_id(self, role_id: int):
        """
        Sets the role_id of this DomainRole.

        :param role_id: The role_id of this DomainRole.
        :type role_id: int
        """
        if role_id is None:
            raise ValueError("Invalid value for `role_id`, must not be `None`")

        self._role_id = role_id

    @property
    def grant_implicitly(self) -> bool:
        """
        Gets the grant_implicitly of this DomainRole.

        :return: The grant_implicitly of this DomainRole.
        :rtype: bool
        """
        return self._grant_implicitly

    @grant_implicitly.setter
    def grant_implicitly(self, grant_implicitly: bool):
        """
        Sets the grant_implicitly of this DomainRole.

        :param grant_implicitly: The grant_implicitly of this DomainRole.
        :type grant_implicitly: bool
        """
        if grant_implicitly is None:
            raise ValueError("Invalid value for `grant_implicitly`, must not be `None`")

        self._grant_implicitly = grant_implicitly

    @property
    def created_at(self) -> datetime:
        """
        Gets the created_at of this DomainRole.

        :return: The created_at of this DomainRole.
        :rtype: datetime
        """
        return self._created_at

    @created_at.setter
    def created_at(self, created_at: datetime):
        """
        Sets the created_at of this DomainRole.

        :param created_at: The created_at of this DomainRole.
        :type created_at: datetime
        """
        if created_at is None:
            raise ValueError("Invalid value for `created_at`, must not be `None`")

        self._created_at = created_at

    @property
    def updated_at(self) -> datetime:
        """
        Gets the updated_at of this DomainRole.

        :return: The updated_at of this DomainRole.
        :rtype: datetime
        """
        return self._updated_at

    @updated_at.setter
    def updated_at(self, updated_at: datetime):
        """
        Sets the updated_at of this DomainRole.

        :param updated_at: The updated_at of this DomainRole.
        :type updated_at: datetime
        """
        if updated_at is None:
            raise ValueError("Invalid value for `updated_at`, must not be `None`")

        self._updated_at = updated_at

