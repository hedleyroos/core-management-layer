...
CODEGEN_VERSION=3.0.2
CODEGEN=java -jar openapi-generator-cli-$(CODEGEN_VERSION).jar generate
...
docs-build:  $(VENV)
	...
	$(VENV)/bin/sphinx-apidoc -f management_layer -o docs/
...
openapi-generator-cli-$(CODEGEN_VERSION).jar:
	curl http://central.maven.org/maven2/org/openapitools/openapi-generator-cli/$(CODEGEN_VERSION)/openapi-generator-cli-$(CODEGEN_VERSION).jar -o openapi-generator-cli-$(CODEGEN_VERSION).jar
...
user-data-store-client: openapi-generator-cli-$(CODEGEN_VERSION).jar swagger/user_data_store.yml
	echo "Generating the client for the User Data Store API..."
	$(CODEGEN) --generator-name python --library asyncio -i swagger/user_data_store.yml -D packageName=user_data_store -o /tmp/$(USER_DATA_STORE_CLIENT_DIR)
...
access-control-client: openapi-generator-cli-$(CODEGEN_VERSION).jar swagger/access_control.yml
	echo "Generating the client for the Access Control API..."
	$(CODEGEN) --generator-name python --library asyncio -i swagger/access_control.yml -D packageName=access_control -o /tmp/$(ACCESS_CONTROL_CLIENT_DIR)
...
authentication-service-client: openapi-generator-cli-$(CODEGEN_VERSION).jar swagger/authentication_service.yml
	echo "Generating the client for the Authentication Service API..."
	$(CODEGEN) --generator-name python --library asyncio -i swagger/authentication_service.yml -D packageName=authentication_service -o /tmp/$(AUTHENTICATION_SERVICE_CLIENT_DIR)
